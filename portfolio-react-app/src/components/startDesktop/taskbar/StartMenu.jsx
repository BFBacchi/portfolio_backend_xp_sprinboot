...
const StartMenu = ({ open, onClose, anchorRef }) => {
  ...
  const { openOverlay, openMyDocuments, openExplorerAt, runCommand } = useDesktopActions();
  ...
  const items = useMemo(
    ...
    [openOverlay, openMyDocuments, openExplorerAt, runCommand]
  );
          ...
          <footer className="start-menu-xp-footer">
            <button
              ...
              onClick={() => {
                if (
                  window.confirm(
                    "¿Desea cerrar la sesión?\n(En esta demo solo se muestra el mensaje.)"
                  )
                ) {
                  window.alert("Cerrar sesión (demostración del portfolio).");
                }
                onClose();
              }}
            >
            ...
            <button
              ...
              onClick={() => {
                window.alert(
                  "Apagar el equipo (demostración).\nCierre la pestaña del navegador para salir del portfolio."
                );
                onClose();
              }}
            >
